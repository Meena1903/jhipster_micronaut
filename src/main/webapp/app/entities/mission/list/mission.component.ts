...
import { EntityArrayResponseType, MissionService } from '../service/mission.service';
...
export class MissionComponent implements OnInit {
  ...
  trackId = (_index: number, item: IMission): number => this.missionService.getMissionIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.missions = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IMission[] | null): IMission[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.missionService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
