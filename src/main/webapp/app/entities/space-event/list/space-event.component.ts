...
import { EntityArrayResponseType, SpaceEventService } from '../service/space-event.service';
...
export class SpaceEventComponent implements OnInit {
  ...
  trackId = (_index: number, item: ISpaceEvent): number => this.spaceEventService.getSpaceEventIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.spaceEvents = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ISpaceEvent[] | null): ISpaceEvent[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const spaceEventsNew = this.spaceEvents ?? [];
      if (data) {
        for (const d of data) {
          if (spaceEventsNew.map(op => op.id).indexOf(d.id) === -1) {
            spaceEventsNew.push(d);
          }
        }
      }
      return spaceEventsNew;
    }
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.spaceEventService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
